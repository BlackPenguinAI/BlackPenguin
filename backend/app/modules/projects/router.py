...
from . import meta_service, services, source_service
...
from .schemas import (
    ...
    ProjectProfileResponse, ProjectResponse, ProposalDecision, ProposalDecisionResponse,
...
def list_projects(db: Session = Depends(get_db), current_user: User = Depends(RoleChecker(VIEWER_ROLES))):
    projects = db.query(Project).options(joinedload(Project.profile)).filter(Project.company_id == current_user.company_id).all()
...
def get_chat(project_id: str, db: Session = Depends(get_db), current_user: User = Depends(RoleChecker(EDITOR_ROLES))):
    ...
    messages = db.query(ProjectMessage).filter(ProjectMessage.session_id == project.session.id).order_by(ProjectMessage.created_at.asc()).all()
    return [{"sender": "user" if item.sender == SenderType.USER else "ai", "content": item.content, "created_at": item.created_at} for item in messages]
...
async def send_chat(project_id: str, payload: ChatMessagePayload, db: Session = Depends(get_db), current_user: User = Depends(RoleChecker(EDITOR_ROLES))):
    ...
    services.save_message(db, project.session.id, SenderType.USER, payload.message)

    sources, source_errors = [], []
    for raw_url in URL_PATTERN.findall(payload.message)[:3]:
        url = raw_url.rstrip(".,;:!?)\"]}")
        try:
            sources.append(await source_service.ingest_url(db, project_id=project.id, user_id=current_user.id, url=url))
        except HTTPException as exc:
            source_errors.append({"url": url, "error": exc.detail})

    history = db.query(ProjectMessage).filter(ProjectMessage.session_id == project.session.id).order_by(ProjectMessage.created_at.asc()).all()
    company_profile = db.query(CompanyProfile).filter(CompanyProfile.company_id == current_user.company_id).first()
    ...
    try:
        raw = await generate_llm_response(ai_config.openrouter_api_key, model, messages, response_format={"type": "json_object"}, temperature=0.25, raise_on_error=True)
        parsed = _parse_agent_response(raw)
        if parsed is None:
            repaired = await generate_llm_response(ai_config.openrouter_api_key, model, messages + [{"role": "assistant", "content": raw}, {"role": "system", "content": "Repair the response and return only valid contract JSON."}], response_format={"type": "json_object"}, temperature=0, raise_on_error=True)
    ...
    result = services.apply_field_updates(
        db, profile, updates, allow_authoritative_statuses=current_user.role == UserRole.ADMIN, final_approved=approved
    )
    if result.accepted:
    ...
    return {
        "message": _message(ai_message), "profile": services.serialize_profile(profile),
        "accepted_fields": [item["field"] for item in result.accepted],
        "rejected_updates": [{"field": item.get("update", {}).get("field"), "reason": item["reason"]} for item in result.rejected],
...
async def add_files(project_id: str, files: list[UploadFile] = File(...), db: Session = Depends(get_db), current_user: User = Depends(RoleChecker([UserRole.ADMIN]))):
    ...
    sources = [await source_service.ingest_file(db, project_id=project_id, user_id=current_user.id, upload=file) for file in files]
    return [source_service.serialize_source(source) for source in sources]
