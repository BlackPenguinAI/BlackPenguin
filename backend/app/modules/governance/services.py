...
from app.integrations.firebase_admin_client import email_status
from app.modules.system_settings.models import FirebaseConfig, LegalDocument
...
def _appointment_ics(meeting: Meeting, project: Project) -> str:
    ...
    legacy = resolve_visit_location(project)
    address = meeting.visit_address or (legacy["address"] if legacy else None)
    location = ", ".join(value for value in (project.name, address) if value).replace(",", "\\,")
    return "\r\n".join([
...
def process_appointment_email_outbox(db: Session, *, limit: int = 10) -> int:
    query = db.query(AppointmentEmailOutbox).filter(
        AppointmentEmailOutbox.status.in_(["pending", "queued"]),
    ...
    for item in query.order_by(AppointmentEmailOutbox.created_at).limit(limit).all():
        if item.status == "queued" and item.provider_message_id:
            try:
                transport = db.query(FirebaseConfig).first()
                delivery = email_status(project_id=(transport.project_id if transport and transport.project_id else settings.FIREBASE_PROJECT_ID), document_id=item.provider_message_id, mail_collection=(transport.appointment_mail_collection if transport else "mail"))
                state = str(delivery.get("status") or "queued").lower()
                item.delivery_status = state
                if state in {"success", "sent", "delivered"}:
                    item.status = "sent"; item.sent_at = datetime.utcnow(); item.delivered_at = item.sent_at; sent += 1
                elif state in {"error", "failed"}:
                    item.status = "failed"; item.last_error = str(delivery.get("error") or "firebase_delivery_failed")[:120]
        ...
        legacy = resolve_visit_location(project)
        address = meeting.visit_address or (legacy["address"] if legacy else None)
        location = ", ".join(value for value in (project.name, address) if value)
        subject = f"Appointment confirmed · {project.name}"
        when = f"{local_time.strftime('%A, %B %d, %Y at %I:%M %p')} ({project.timezone or 'UTC'})"
        if item.recipient_kind == "lead":
            body = (f"Hello {lead.full_name},\n\nYour property visit is confirmed.\nProject: {project.name}\n"
                    f"Date and time: {when}\nLocation: {location}\nSales representative: {sales_name}\n\n"
                    "The attached calendar invitation contains the same confirmed details.")
            action = "Please arrive at the confirmed location and contact your Sales representative if your plans change."
        else:
            body = (f"A property visit has been assigned to you.\n\nLead: {lead.full_name}\nPhone: {lead.phone}\n"
                    f"Email: {lead.email or 'Not provided'}\nProject: {project.name}\nDate and time: {when}\nLocation: {location}\n\n"
                    f"Prepare in Black Penguin: {settings.PUBLIC_APP_URL.rstrip('/')}/app/schedule?meeting={meeting.id}\n"
                    f"Lead profile: {settings.PUBLIC_APP_URL.rstrip('/')}/app/leads?lead={lead.id}")
            action = "Review the lead profile and visit playbook before the meeting."
        html_body = (
            "<div style=\"font-family:Arial,sans-serif;background:#090909;color:#f5f5f5;padding:28px\">"
            f"<h2 style=\"color:#e9ad00\">{html.escape(subject)}</h2>"
            f"<p style=\"white-space:pre-line;line-height:1.6\">{html.escape(body)}</p>"
            f"<p style=\"color:#bbb\">{html.escape(action)}</p></div>"
        )
        try:
            transport = db.query(FirebaseConfig).first()
            if transport and not transport.appointment_email_enabled:
                raise RuntimeError("Appointment email transport is disabled.")
            configured_from = (
                f"{transport.appointment_from_name} <{transport.appointment_from_email}>"
                if transport and transport.appointment_from_email else None
            )
            result = send_appointment_email(
                document_id=item.id, recipient=item.recipient_email, subject=subject, body=body, html=html_body,
                ics_content=_appointment_ics(meeting, project),
                from_email=configured_from,
                reply_to=transport.appointment_reply_to if transport else None,
                mail_collection=transport.appointment_mail_collection if transport else "mail",
                project_id=transport.project_id if transport else None,
            )
            item.provider_message_id = str(result.get("document_id") or item.id)
            item.delivery_status = str(result.get("status") or "queued")
            item.status = "queued"; item.last_error = None
        except Exception as exc:
            item.last_error = type(exc).__name__; item.attempts += 1
            if item.attempts >= 5:
                item.status = "failed"
