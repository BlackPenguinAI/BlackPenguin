...
export class SmtpConfigComponent implements OnInit {
  ...
  private getUrl() {
    return isDevMode() ? 'http://localhost:8000/api/v1/system/smtp-config' : 'https://blackpenguin.ai/api/v1/system/smtp-config'; 
  }
  ...
  ngOnInit() {
    ...
    this.http.get<any>(this.getUrl(), { headers: this.getHeaders() }).subscribe({
      ...
      error: () => console.log('Aún no hay configuración SMTP guardada.')
    });
  }
  ...
  onSave() {
    ...
    this.http.put(this.getUrl(), this.config, { headers: this.getHeaders() }).subscribe({
      ...
      error: (err) => {
        this.isSaving = false;
        this.isError = true;
        this.statusMessage = 'Error al guardar. Verifica tu conexión.';
        console.error(err);
      }
    });
  }
